...
def preprocess():
    ...
    train_data, temp_data, train_label, temp_label =\
     train_test_split(train_data, train_label, test_size=.83, random_state=42)


    test_data, validation_data, test_label, validation_label =\
     train_test_split(temp_data, temp_label, test_size=.50, random_state=42)
...
print('\n Training set Accuracy:' + str(100 * np.mean((predicted_label == (train_label.T)).astype(float))) + '%')

predicted_label = nnPredict(w1, w2, validation_data)

# find the accuracy on Validation Dataset

print('\n Validation set Accuracy:' + str(100 * np.mean((predicted_label == (validation_label).T).astype(float))) + '%')

predicted_label = nnPredict(w1, w2, test_data)

# find the accuracy on Validation Dataset

print('\n Test set Accuracy:' + str(100 * np.mean((predicted_label == (test_label).T).astype(float))) + '%')
